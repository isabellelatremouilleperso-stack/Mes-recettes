...
# --- AJOUTER (LES 3 ONGLETS) ---
    ...
    with tab2:
        with st.form("vrac_form"):
            ...
            v_txt = st.text_area("Collez tout ici (ingrédients et étapes)", height=250)
            if st.form_submit_button("🚀 Sauver Vrac"):
                if v_t:
                    send_action({"action": "add", "titre": v_t, "categorie": v_cat, "ingredients": v_txt, "preparation": "Vrac", "date": datetime.now().strftime("%d/%m/%Y")})
                    st.session_state.page = "home"; st.rerun()
...
# --- ÉPICERIE / PLANNING / AIDE (Reste inchangé) ---
elif st.session_state.page == "shop":
    st.header("🛒 Ma Liste"); st.write("...chargement...")
elif st.session_state.page == "planning":
    st.header("📅 Planning"); st.write("...chargement...")
elif st.session_state.page == "help":
    st.title("❓ Aide")
    st.write("Utilisez les 3 onglets dans 'Ajouter' pour créer vos recettes selon votre source.")
