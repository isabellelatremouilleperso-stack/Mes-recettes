...
st.markdown("""
    ...
    [data-testid="stImage"] img {
        object-fit: cover;
        height: 200px !iimport streamlit as st
import requests
import pandas as pd
from datetime import datetime
...
with st.sidebar:
    ...
    if st.button("🛒 Liste d'épicerie", use_container_width=True):
        ...
        st.rerun()
    st.write("---")
    st.metric("À acheter", len(st.session_state.shopping_list))

# 4. LOGIQUE DES PAGES

# --- PAGE DÉTAILS (CORRIGÉE) ---
if st.session_state.page == "details" and st.session_state.recipe_data:
    ...
    with col2:
        ...
        # CORRECTION ACCENT ICI
        st.info(res.get('Préparation', res.get('Preparation', 'Pas d\'étapes indiquées')))

# --- PAGE BIBLIOTHÈQUE (ANTI-NAN) ---
elif st.session_state.page == "home":
    st.header("📚 Ma Bibliothèque")
    try:
        # On remplace les nan par du vide
...
# (Garder tes blocs 'ajouter' et 'shopping' ici)mportant;
        width: 100% !important;
